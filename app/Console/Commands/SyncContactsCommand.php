...
class SyncContactsCommand extends Command
{
    ...
    protected $description = 'Full page-by-page sync of Zoho Contacts into local DB (auto-creates table if needed)';

    public function handle(): int
    {
        $this->info('Checking local DB structure...');

        // If the "contacts" table does not exist — create it via SDK artisan command
        if (!Schema::hasTable('contacts')) {
            ...
            $this->info('Table "contacts" is ready.');
        }

        $page  = 1;
        $total = 0;

        $this->info('Starting full Contacts sync (page-by-page)...');

        while (true) {
            $this->line("Fetching page {$page}...");
            $collection = ContactZoho::all($page);
            ...
            $this->line("Saved {$count} contacts from page {$page}.");
            $page++;
        }
